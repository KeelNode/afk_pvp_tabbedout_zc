...
; Store the hotkeys in variables for display
startHotkey := ""
reloadHotkey := ""
exitHotkey := ""
...
{
    if (action = "StartScript")
        startHotkey := hotkey
    else if (action = "ReloadScript")
        reloadHotkey := hotkey
    else if (action = "ExitApplication")
        exitHotkey := hotkey
}

; Main GUI
; Set dark mode background
Gui, Color, 1a1a1a  ; Dark background color for the GUI

; LEFT COLUMN Heading: MACRO GUIDE for AFK PvP
Gui, Font, S15 Bold, Arial
Gui, Add, Text, y+5 cFF00FF BackgroundTrans  
Gui, Font, S10, Arial  ; Reset font size for separator
Gui, Add, Text, y+10 c6c8197 BackgroundTrans, |===============Read below===============|  ; Gray separator line

; Step 1: Preparation
Gui, Font, S14 , Arial
Gui, Add, Text, y+20 cFF7777, STEP 1: Preparation  ; Bold, red step heading
Gui, Font, S12, Arial
Gui, Add, Text, y+5 c00FF00, - Be In Orbit  ; Green text for instruction
Gui, Add, Text, y+5 c00FF00, - Have Green Launch Button visible  ; Green text for instruction

; Separator
Gui, Font, S10, Arial
Gui, Add, Text,, ..............................................................................  ; Dotted line separator middle left

; Step 2: Setup 
Gui, Font, S14 , Arial
Gui, Add, Text, y+10 cFF7777, STEP 2: Setup  ; 
Gui, Font, S12, Arial
Gui, Add, Text, y+5 cFFFFFF, - MUST HAVE 100 MOBILITY  ; White text for instructions
Gui, Add, Text, y+5 cFFFFFF, - Crucible mode: Private Match 
Gui, Add, Text, y+5 cFFFFFF, - Game Type: Zone Control
Gui, Add, Text, y+5 cFFFFFF, - Game Map: Javelin-4
Gui, Add, Text, y+5 cFFFFFF, - Super Charge Rate: 50x 
Gui, Font, S8
Gui, Add, Text, y+0 c6c8197, ^** Optional but for orbs, use Bubble Titan or Well Warlock
Gui, Font, S12
Gui, Add, Text, y+5 cFFFFFF, - Select Score Limit: 25, 50, or 100
Gui, Add, Text, y+5 cFFFFFF, - Select HDD or SSD

; Separator MIDDLE COLUMN
Gui, Add, Text, y-0 c1a1a1a, .........................
Gui, Add, Text, y+10 cbb00bb, Guide for Private match PvP
Gui, Add, Text, y+5 cbb00bb, Please follow all instructions carefully
; WARNING MACROS
Gui, Add, Text, c1a1a1a, .........................
Gui, Add, Text, y+0 cRed, ************************************************
Gui, Add, Text, y+0 cRed, Due to bans of fellow macro masters
Gui, Add, Text, y+5 cRed, Please use this at your own risk
Gui, Add, Text, y+10 cRed, ************************************************
; Display current macro and hotkeys
Gui, Font, S12, Arial
Gui, Add, Text, y+50 cbb00bb, Current file: %A_ScriptName%  ; Blue text for macro name
Gui, Add, Text, y+5 cbb00bb,%startHotkey% - Start  ; Blue text for hotkey
Gui, Add, Text, y+0 cbb00bb,%reloadHotkey% - Stop/Reload  ; Blue text for hotkey
Gui, Add, Text, y+0 cbb00bb,%exitHotkey% - Quit  ; Blue text for hotkey

; RIGHT COLUMN Heading: Please follow the instructions carefully
Gui, Font, S15 , Arial
Gui, Add, Text, y-0 x+250 cFF00A9 BackgroundTrans  
Gui, Font, S10, Arial  ; Reset font size for separator
Gui, Add, Text, y+10 c6c8197 BackgroundTrans, |===============Select below===============|  ; Gray separator line

; Column 2: Score Limit and Storage Type Selection
Gui, Font, S12, Arial
Gui, Add, Text, cWhite, Select Score Limit:  ; Score Limit label
Gui, Font, S10 cWhite, Arial
Gui, Add, Text, y+0 c6c8197, * Only 50 and above give Crucible Reputation/Valor Points  ; Gray text for info

Gui, Add, Radio, vscoreLimitChoice, 25
Gui, Add, Radio, y+1, 50
Gui, Add, Radio, y+1, 100

Gui, Font, S12, Arial
Gui, Add, Text, y+20 cWhite, Select Storage Type:  ; Storage Type label
Gui, Font, S10 cWhite, Arial
Gui, Add, Text, y+5 c6c8197, * This is for better consistency  ; Gray text for info
Gui, Add, Text, y+0 c6c8197, ** If you are unsure, try both and see which works best  ; Gray text for info
...
if (scoreLimitChoice = 0 || hddSsdChoice = 0) {
    Gui, New, +AlwaysOnTop
    Gui, Font, S12, Arial
    Gui, Add, Text,y+0, Must select an option for * Score Limit and Storage Type *
    Gui, Add, Text,, If you need help, reach out on the Discord
    Gui, Add, Text, y+0 cBlue gOpenDiscord, Here (thrallway)
    Gui, Add, Text,y+0, or
    Gui, Add, Text,y+0 cBlue gOpenDiscordInvite, Here (invite link)
    Gui, Add, Text,y+1, or contact me @keelnode.
    Gui, Add, Button, gCloseErrorMsg, OK
    Gui, Show,, Selection Error
    Return  ; Exit without closing the settings GUI
}

; Convert scoreLimitChoice to its corresponding text value
if (scoreLimitChoice = 1)
    scoreLimit := "25"
else if (scoreLimitChoice = 2)
    scoreLimit := "50"
else if (scoreLimitChoice = 3)
    scoreLimit := "100"

; Convert hddSsdChoice to its corresponding text value
if (hddSsdChoice = 1)
    hddSsd := "SSD"
else if (hddSsdChoice = 2)
    hddSsd := "HDD"

Gui, Destroy  ; Close the settings GUI after saving
...
; Function to handle the click on the second Discord invite link
OpenDiscordInvite:
Run, https://discord.com/invite/KGyjysA5WY
Return

; Function to close the custom error message GUI
CloseErrorMsg:
Gui, Destroy
Return
...
StartScript:
    ...
    Loop {
        ...
        ; Use the determined delay
        ShowTooltipCountdown("Delaying after launch...", delay)
        ...
        ; Super for orbs at Flag A
        ShowTooltip("Using Super at Flag A")
        ...
        360Controller.Axes.LY.SetState(50)
        Sleep, 200
        ; super for orbs at Flag C
        ShowTooltip("Using Super at Flag C")
        360Controller.Buttons.RB.SetState(true)
        360Controller.Buttons.LB.SetState(true)
        PreciseSleep(100)
        360Controller.Buttons.RB.SetState(false)
        ...
        ShowTooltip("Moving from Flag C")
        360Controller.Axes.LX.SetState(0)
        PreciseSleep(2850)
        360Controller.Axes.LX.SetState(50)
        Sleep, 200
        ...
        ; Super Spam ShowTooltip("Super Spamming at Flag B")
        Gosub, SuperSpam
        ; Back to Orbit
        
        ; ShowTooltipCountdown("Final super before going Orbit in 2 seconds ...", 2000)
        ; 360Controller.Buttons.RB.SetState(true)
        ; 360Controller.Buttons.LB.SetState(true)
        ; PreciseSleep(100)
        ; 360Controller.Buttons.RB.SetState(false)
...
SuperSpam:
    ...
    Loop, %superCount%
    {
        ...
        superIndex := A_Index
        totalSeconds := 11  ; Total duration for the countdown (10 seconds sleep)

        ; Countdown before performing the super
        Loop, %totalSeconds%
        {
            tooltipText := "Super " superIndex "/" superCount " in progress...`n in " (totalSeconds - A_Index + 1) " seconds..."
            Tooltip, %tooltipText%
            Sleep, 1000
        }
        ...
        ; Shuffle back and forward to avoid AFK (only for 50+ score limits)
        if (scoreLimit = 50)
        {
            360Controller.Axes.LY.SetState(0)
            PreciseSleep(100)
        ...
        ; Small delay after each super action
